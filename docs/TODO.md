...
- done: the repo now documents a Linux-friendly local baseline of Node.js `20.x`, npm `10.x`, `.nvmrc`, and global `@zeppos/zeus-cli` installation so fresh machines do not need guesswork,
- note: the Playwright module-harness scripts can run on Linux, but they require `PLAYWRIGHT_COVERAGE_BROWSER` to point at a local Chromium-family browser because `playwright-core` does not bundle one here,
- done: the repo now includes `scripts/playwright-flatpak-host-browser.sh` so Flatpak-hosted VS Code or Codex sessions can launch host Chromium with forwarded remote-debugging pipe file descriptors during Playwright harness runs,
