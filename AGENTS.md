...
- Keep `shared/i18n/phone-locale.js` browser-safe. Browser harnesses and Playwright module coverage should not import `@zos/settings` indirectly through phone-side storage helpers.
- Keep Zepp-only locale resolution in `shared/i18n/watch-locale.js`. Watch pages and bootstrap locale hints may use it, but browser-safe shared modules should not.
- Starter recipe localization now lives in split locale modules under `shared/domain/seed-library/`. Keep additions per locale file and let `seed-library.js` compose them instead of duplicating the whole localized catalog inline.
...
- Round page layouts now also apply a conservative compact-round tuning when runtime sizing reports a sub-`480x480` round screen. Treat that as the first compatibility layer for Balance 1-class hardware; do not fork a second round page set unless real-device validation shows the compact pass is still insufficient.
...
- On Windows, Codex or other agent-driven shells may expose the cwd as a namespaced path like `\\?\C:\...`; in that environment, prefer the repo wrappers `npm run zepp:dev -- ...` and `npm run zepp:preview -- ...` instead of raw `zeus dev` or `zeus preview`, because Zeus may mis-handle that cwd while a normal VS Code terminal still works.
