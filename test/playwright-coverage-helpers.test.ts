...
describe("playwright coverage helpers", () => {
  it("parses default coverage args", () => {
  ...
  it("parses explicit coverage args", () => {
    expect(
      parsePlaywrightCoverageArgs(["--duration-ms", "5000", "--output-dir", "coverage/custom", "--verbose"])
    ).toEqual({
      mode: "simulator",
      durationMs: 5000,
  ...
  it("switches to module-harness mode and harness output dir", () => {
    expect(parsePlaywrightCoverageArgs(["--mock-page"])).toEqual({
      mode: "module-harness",
      durationMs: DEFAULT_PLAYWRIGHT_COVERAGE_DURATION_MS,
  ...
  it("adds extra roots for mock-page coverage", () => {
  ...
  it("prefers an override browser path for mock coverage", () => {
    expect(
      getMockBrowserExecutableCandidates({
